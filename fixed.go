...
func Fixed(namespace string, values map[string]interface{}) Source {
...
func (s fixedSource) GetString(key string) string {
	if def, ok := s.defaults[joinKey(s.namespace, key)].(string); ok {
		return def
	}
	return ""
}
...
func (s fixedSource) GetInt(key string) int {
	if def, ok := s.defaults[joinKey(s.namespace, key)].(int); ok {
		return def
	}
	return 0
}
...
func (s fixedSource) GetUint(key string) uint {
	if def, ok := s.defaults[joinKey(s.namespace, key)].(uint); ok {
		return def
	}
	return uint(0)
}
